...
#if defined(CUDASSL_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
#if defined(CUDASSL_AESNI_C) && defined(CUDASSL_HAVE_X86_64)
    if (aesni_supports(CUDASSL_AESNI_AES))
                    ...
                    unsigned char *output)
{
    ...
    if (length % 16)
        return(CUDASSL_ERR_AES_INVALID_INPUT_LENGTH);
 ...
 * Checkup routine
 */
extern "C" int aes_self_test(int verbose)
{
    int ret = 0, i, j, u, v;
...
#if defined(CUDASSL_CIPHER_MODE_CBC) || \
    defined(CUDASSL_CIPHER_MODE_CTR) || \
    defined(CUDASSL_CIPHER_MODE_CFB)
    unsigned char iv[16];
#endif
...
#endif
#if defined(CUDASSL_CIPHER_MODE_CTR) || defined(CUDASSL_CIPHER_MODE_CFB)
    size_t offset;
#endif
#if defined(CUDASSL_CIPHER_MODE_CTR)
    int len;
