 ...
 */
__constant__ static unsigned char md5_test_buf[7][81] =
{
    { "" },
    { "a" },
    { "abc" },
...
__constant__ static const int md5_test_buflen[7] =
{
    0, 1, 3, 14, 26, 62, 80
};
 ...
 * Checkup routine
 */
__global__ void md5_self_test_kernel(unsigned char *outputs)
{
    int i;
    unsigned char *md5sum;

    for (i = 0; i < 6; i++) {
        md5sum = outputs + (TX * 6 + i) * MD5_DIGEST_LENGTH;

        md5(md5_test_buf[i], md5_test_buflen[i], md5sum);
    }
}

int md5_self_test(int verbose, cuda_device *d) {
  int i; 
  unsigned char ret[2][3][MD5_DIGEST_LENGTH];

  md5_self_test_kernel<<<1, 2>>>(d->device_data_out);
  cuda_download_data(ret, d->device_data_out, 2 * 3 * MD5_DIGEST_LENGTH);

  for (i = 0; i < 6; i++) {
    if (verbose != 0)
      printf("  MD5 test #%d: ", i + 1);

    if (memcmp(ret[0][i], md5_test_sum[i], MD5_DIGEST_LENGTH) != 0 &&
      memcmp(ret[1][i], md5_test_sum[i], MD5_DIGEST_LENGTH) != 0) {
      if (verbose != 0)
        printf("failed\n");
    } else {
      if (verbose != 0)
...
__global__ void md5_performance_test_kernel() {
  int i;
  unsigned char src[8];
  unsigned char ret[MD5_DIGEST_LENGTH];
  md5_context ctx;
  ...
  for (i = 0; i < LOOPS; i++)
    md5_update(&ctx, src, 8);

  md5_finish(&ctx, ret);
  md5_free(&ctx);
}

extern "C" int md5_performance_test(int verbose, cuda_device *d) {
  int i; float h;

  CUDA_START_TIME

  // for (i = 0; i < LOOPS; ++i) {
  md5_performance_test_kernel<<<DATASIZE, MAX_THREAD>>>();
  // }
